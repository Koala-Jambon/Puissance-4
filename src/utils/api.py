...
class Game:
    ...
    def check_victory_h(self):
        ...
        for check_x, check_y in tool.product(range(4), range(6)):
            n = 0
            while self.board[check_y][check_x + n] == self.board[check_y][check_x] and self.board[check_y][
                check_x] != 0:
                if n != 3:
                    n += 1
                else:
                    ...
                    self.victory_reason = (
                    (check_x, check_y), (check_x + 1, check_y), (check_x + 2, check_y), (check_x + 3, check_y))
                    return int(winner)
        return False
    ...
    def check_victory_v(self):
        ...
        for check_x, check_y in tool.product(range(7), range(3)):
            n = 0
            while self.board[check_y + n][check_x] == self.board[check_y][check_x] and self.board[check_y][
                check_x] != 0:
                if n != 3:
                    n += 1
                else:
                    winner = self.board[check_y][check_x]
                    self.victory_reason = (
                    (check_x, check_y), (check_x, check_y + 1), (check_x, check_y + 2), (check_x, check_y + 3))
                    return int(winner)
        return False
    ...
    def check_victory_dp(self):
        ...
        for check_x, check_y in tool.product(range(4), range(3)):
            n = 0
            while self.board[check_y + n][check_x + n] == self.board[check_y][check_x] and self.board[check_y][
                check_x] != 0:
                if n != 3:
                    n += 1
                else:
                    winner = self.board[check_y][check_x]
                    self.victory_reason = ((check_x, check_y), (check_x + 1, check_y + 1), (check_x + 2, check_y + 2),
                                           (check_x + 3, check_y + 3))
                    return int(winner)
        return False
