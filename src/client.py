...
class App:
    ...
    def update_get_username(self):
        pyxel_key_letters = [pyxel.KEY_A, pyxel.KEY_B, pyxel.KEY_C, pyxel.KEY_D, pyxel.KEY_E, pyxel.KEY_F, pyxel.KEY_G,
    ...
    def update_choose_party(self):
        ...
        elif pyxel.btnp(pyxel.KEY_DOWN) and (
                self.party_choice_number != self.party_infos()["number"] - 1 or self.party_choice_number == -1):
            self.party_choice_number += 1
        ...
        if self.party_choice_number != -1:
            for draw_button in range(len(buttons_coords["x"])):
                ...
                if button_constant in self.party_infos()["empty"]:
                    self.draw_text(f"{button_constant}-Empty",
                                   (int(510 / size), int(buttons_coords["y"][draw_button] + 50 / size)))
                elif button_constant in self.party_infos()["free"]:
                    try:
                        self.draw_text(f"{button_constant}-{self.party_infos()['players_list'][button_constant - 1]}",
                                       (int(510 / size), int(buttons_coords["y"][draw_button] + 50 / size)))
                    except:
                        self.draw_text(f"{button_constant}-Error",
                                       (int(510 / size), int(buttons_coords["y"][draw_button] + 50 / size)))
                elif button_constant <= self.party_infos()["number"]:
                    self.draw_text(f"{button_constant}-Full",
                                   (int(510 / size), int(buttons_coords["y"][draw_button] + 50 / size)))
                if draw_button == self.party_choice_number % 3:
                    for draw_w, draw_h in tool.product(range(buttons_coords["w"][draw_button]),
                                                       range(buttons_coords["h"][draw_button])):
                        if pyxel.pget(buttons_coords["x"][draw_button] + draw_w,
                                      buttons_coords["y"][draw_button] + draw_h) == 0:
                            pyxel.pset(buttons_coords["x"][draw_button] + draw_w,
                                       buttons_coords["y"][draw_button] + draw_h, 5)
            pyxel.rect(20 / size, 20 / size, int(100 / size), int(100 / size), 2)
        else:
            for draw_button in [1, 2, 3]:
                if draw_button in self.party_infos()["empty"]:
                    self.draw_text(f"{draw_button}-Empty",
                                   (int(510 / size), int(buttons_coords["y"][draw_button - 1] + 50 / size)))
                elif draw_button in self.party_infos()["free"]:
                    try:
                        self.draw_text(f"{draw_button}-{self.party_infos()['players_list'][draw_button - 1]}",
                                       (int(510 / size), int(buttons_coords["y"][draw_button - 1] + 50 / size)))
                    except:
                        self.draw_text(f"{draw_button}-Error",
                                       (int(510 / size), int(buttons_coords["y"][draw_button - 1] + 50 / size)))
                elif draw_button <= self.party_infos()["number"]:
                    self.draw_text(f"{draw_button}-Full",
                                   (int(510 / size), int(buttons_coords["y"][draw_button - 1] + 50 / size)))
        ...
        for draw_button in range(len(buttons_coords["x"])):
            if draw_button == self.button:
                for draw_w, draw_h in tool.product(range(buttons_coords["w"][draw_button]),
                    ...
                    if pyxel.pget(buttons_coords["x"][draw_button] + draw_w,
                                  buttons_coords["y"][draw_button] + draw_h) == 0:
                        pyxel.pset(buttons_coords["x"][draw_button] + draw_w, buttons_coords["y"][draw_button] + draw_h,
                                   5)
            else:
                for draw_w, draw_h in tool.product(range(buttons_coords["w"][draw_button]),
                                                   range(buttons_coords["h"][draw_button])):
            ...
            if (self.game.player_turn_number == self.player_number):
                ...
                if "/waitgame" == data["message"]:
                    ...
                    pyxel.circ((150 * (self.choice_position - 1) + 510) / size, 75 / size, 70 / size,
                               2 * (self.game.player_turn_number - 1) + 8)
                else:
                    self.choice_position = 0
                    # Updates the board
                    self.game.board = data["board"]
    ...
    def draw_text(self, text: str, coords: tuple):
        ...
        for letter in text:
            try:
                ...
                pyxel.blt(coords[0], coords[1], letters_coords[letter][1], letters_coords[letter][4],
                          letters_coords[letter][5], letters_coords[letter][2], letters_coords[letter][3])
